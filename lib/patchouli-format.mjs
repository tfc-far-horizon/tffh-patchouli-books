...
function jsonFence(language, value) {
...
function renderPage(page) {
  ...
  if (page.type === 'patchouli:text') {
    ...
    if (page.text) lines.push(...patchouliTextToIgem(page.text).split('\n').map((line) => `\t${line}`));
  } else if (page.type === 'tfc:multimultiblock') {
    // This page contains nested multiblock data. JSON is the lossless form and
    // is understood by the existing format parser as raw/<page-type>.
    lines.push(`\t${jsonFence(`raw/${page.type}`, page).replaceAll('\n', '\n\t')}`);
  } else if (page.type.startsWith('raw/')) {
    lines.push(`\t${jsonFence(page.type, page).replaceAll('\n', '\n\t')}`);
